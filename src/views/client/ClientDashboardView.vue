  ...
  <div class="client-page">
    ...
    <header class="client-topbar">
      <div class="topbar-brand">
        <div class="topbar-logo"><img src="/images/logo.webp" alt="Logo" /></div>
        <span class="topbar-text">Mengundang<span>Anda</span></span>
      </div>
      <div class="topbar-right">
        <span v-if="invitation" class="topbar-names">{{ invitation.groom_name }} & {{ invitation.bride_name }}</span>
    ...
    <div v-if="toast" :class="['toast', `toast-${toast.type}`]">
      <span class="material-symbols-rounded" style="font-size:18px">{{ toast.type === 'error' ? 'error' : 'check_circle' }}</span>
      {{ toast.message }}
    </div>
  </div>
</template>
