<template>
  <div class="invitation-page theme-elegant">
    <!-- COVER -->
...
/* ===== ELEGANT GOLD THEME ===== */
.theme-elegant {
  --inv-bg: #faf8f4; --inv-primary: #8b6f4e; --inv-primary-light: #c4a476; --inv-secondary: #d4a574;
  --inv-gold: #c9a96e; --inv-gold-light: rgba(201,169,110,0.15); --inv-dark: #2c2417;
  --inv-text: #3d3425; --inv-text-light: #7a6e5d; --inv-cream: #f5efe6; --inv-white: #fff;
...
.inv-cover { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; position: relative; background: linear-gradient(180deg, rgba(44,36,23,0.6) 0%, rgba(44,36,23,0.8) 100%); overflow: hidden; }
.inv-cover-bg { position: absolute; inset: 0; z-index: 0; }
.inv-cover-bg img { width: 100%; height: 100%; object-fit: cover; }
.inv-cover-fallback { width: 100%; height: 100%; background: linear-gradient(135deg, #2c2417, #4a3a28); }
.inv-cover-content { position: relative; z-index: 1; padding: 40px 20px; }
.inv-cover .inv-label { font-family: var(--font-sans); font-size: 14px; letter-spacing: 6px; text-transform: uppercase; color: var(--inv-gold); margin-bottom: 24px; }
...
.inv-section-title { font-family: var(--font-script); font-size: clamp(32px,6vw,48px); color: var(--inv-primary); margin-bottom: 8px; }
.inv-section-divider { display: flex; align-items: center; justify-content: center; gap: 16px; margin-bottom: 40px; color: var(--inv-gold); }
.inv-section-divider span { font-size: 24px; }
.inv-section-divider::before, .inv-section-divider::after { content: ''; width: 60px; height: 1px; background: var(--inv-gold); }
...
.profile-photo { width: 180px; height: 180px; border-radius: 50%; object-fit: cover; border: 4px solid var(--inv-gold); margin: 0 auto 20px; box-shadow: 0 8px 30px rgba(139,111,78,0.15); }
.profile-photo-placeholder { width: 180px; height: 180px; border-radius: 50%; margin: 0 auto 20px; display: flex; align-items: center; justify-content: center; background: var(--inv-cream); border: 4px solid var(--inv-gold); font-size: 60px; color: var(--inv-gold); }
...
.event-card { background: var(--inv-white); padding: 32px; border-radius: 16px; box-shadow: 0 4px 20px rgba(139,111,78,0.08); border: 1px solid rgba(201,169,110,0.2); }
...
.event-map-link { display: inline-flex; align-items: center; gap: 6px; margin-top: 12px; padding: 8px 16px; border: 1px solid var(--inv-gold); border-radius: 8px; color: var(--inv-primary); font-size: 13px; transition: all 0.3s; text-decoration: none; }
.event-map-link:hover { background: var(--inv-gold); color: #fff; }
.inv-story { background: var(--inv-white); }
.story-timeline { position: relative; max-width: 600px; margin: 0 auto; }
.story-timeline::before { content: ''; position: absolute; left: 50%; top: 0; bottom: 0; width: 2px; background: var(--inv-gold); transform: translateX(-50%); }
...
.story-dot { width: 16px; height: 16px; background: var(--inv-gold); border-radius: 50%; border: 3px solid var(--inv-bg); z-index: 1; }
...
.rsvp-form .form-input { width: 100%; padding: 14px 16px; border: 1px solid rgba(139,111,78,0.2); border-radius: 8px; background: var(--inv-cream); color: var(--inv-text); font-size: 14px; transition: all 0.3s; }
.rsvp-form .form-input:focus { border-color: var(--inv-gold); box-shadow: 0 0 0 3px var(--inv-gold-light); outline: none; }
.radio-group { display: flex; gap: 12px; }
.radio-option { flex: 1; padding: 14px; border: 1px solid rgba(139,111,78,0.2); border-radius: 8px; text-align: center; cursor: pointer; transition: all 0.3s; font-size: 14px; }
.radio-option.active { border-color: var(--inv-gold); background: var(--inv-gold-light); color: var(--inv-primary); font-weight: 500; }
.btn-rsvp { width: 100%; padding: 16px; background: var(--inv-primary); color: #fff; font-size: 15px; font-weight: 600; border-radius: 8px; transition: all 0.3s; letter-spacing: 1px; border: none; cursor: pointer; }
.btn-rsvp:hover { background: var(--inv-dark); }
.rsvp-messages { max-width: 500px; margin: 32px auto 0; }
.rsvp-message { padding: 16px; background: var(--inv-cream); border-radius: 8px; margin-bottom: 12px; text-align: left; }
...
.msg-status { font-size: 12px; margin-top: 6px; color: var(--inv-gold); }
.gift-box { display: inline-block; padding: 24px 40px; background: var(--inv-white); border-radius: 16px; border: 1px solid rgba(201,169,110,0.3); text-align: center; }
...
.gift-copy { margin-top: 12px; padding: 8px 20px; background: var(--inv-gold); color: #fff; border-radius: 8px; font-size: 13px; font-weight: 500; border: none; cursor: pointer; }
