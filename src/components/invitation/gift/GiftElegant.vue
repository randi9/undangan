<template>
  ...
  >
    ...
    <div ref="headerRef" class="opacity-0 translate-y-6 mb-12">
      ...
      <div class="flex items-center justify-center gap-4 mb-4">
        <svg viewBox="0 0 60 2" class="w-10 md:w-14" style="opacity: 0.4">
          <rect width="60" height="1" fill="var(--theme-secondary)" />
        </svg>
      ...
      <p
        class="max-w-xl mx-auto text-sm md:text-base leading-relaxed"
    ...
    <div
      class="flex flex-col items-center gap-10 w-full overflow-hidden px-2 py-4"
    >
      <div
      ...
      >
        ...
        <div
          class="group w-full aspect-[1.58] bg-gradient-to-br from-[#ffffff] to-[#fbfaf8] p-6 rounded-2xl shadow-xl border relative overflow-hidden flex flex-col justify-between text-left transition-transform duration-500 hover:scale-[1.03]"
          :style="{
            borderColor: 'rgba(201,169,110,0.3)',
            boxShadow: '0 12px 30px rgba(139,111,78,0.1)',
          }"
        >
          <!-- Moving Gold Shimmer (hover effect) -->
          <div
            class="absolute inset-0 z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-1000 pointer-events-none"
            style="
          ...
          <div
            class="absolute -right-12 -top-12 w-48 h-48 border-[1.5px] rounded-full pointer-events-none"
            :style="{ borderColor: 'rgba(201,169,110,0.15)' }"
          ></div>
          <div
            class="absolute -right-6 -top-6 w-32 h-32 border-[1.5px] rounded-full pointer-events-none"
            :style="{ borderColor: 'rgba(201,169,110,0.2)' }"
          ></div>
          <div
            class="absolute -left-12 -bottom-12 w-40 h-40 border-[1px] rounded-full pointer-events-none"
          ...
          <div class="flex justify-between items-start z-10">
            <!-- EMV Chip -->
            <div
              class="w-11 h-8 rounded-md relative overflow-hidden shadow-sm border border-black/10"
              :style="{
                background:
                  'linear-gradient(135deg, #e5c875, #d4af37, #c9a96e)',
              }"
            >
              ...
              <div
                class="absolute inset-y-2 inset-x-3.5 border border-black/10 rounded-sm"
              ></div>
            </div>

            <div
              class="font-bold text-xl tracking-widest uppercase"
          ...
          <div class="z-10 mt-auto mb-6 relative">
            <div
              class="text-[clamp(18px,5vw,24px)] font-bold tracking-[0.12em] font-mono drop-shadow-sm flex items-center gap-3"
          ...
          <div class="flex justify-between items-end z-10">
            <div class="flex flex-col">
              ...
              >
              <span
                class="text-sm md:text-base font-bold uppercase tracking-widest leading-tight"
                :style="{ color: 'var(--theme-primary)' }"
              >
                {{ bank.bank_holder }}
              </span>
            ...
            <div class="flex opacity-50 items-center drop-shadow-sm">
              <div
                class="w-8 h-8 rounded-full mix-blend-multiply"
                :style="{ backgroundColor: 'var(--theme-secondary)' }"
              ></div>
              <div
                class="w-8 h-8 rounded-full mix-blend-multiply -ml-4"
      ...
      <div
        ...
        :ref="
          (el) => {
            if (el) cardRefs[bankList.length] = el as HTMLElement;
          }
        "
        class="flex flex-col items-center gap-6 w-full max-w-[400px] opacity-0 translate-y-12"
      >
        <div
          class="group w-full p-6 rounded-2xl shadow-xl border relative overflow-hidden flex flex-col justify-between text-left transition-transform duration-500 hover:scale-[1.03]"
          :style="{
        ...
        >
          ...
          <div class="flex justify-between items-start z-10 mb-4">
            <div
              class="font-bold text-lg tracking-wider uppercase"
              :style="{ color: 'var(--theme-primary)' }"
            >
              Kirim Kado
            </div>
          ...
          <div class="z-10 mb-4">
            <p
              class="text-sm text-gray-700 leading-relaxed font-medium mb-3"
              style="white-space: pre-line"
            >
              {{ invitation.gift_address }}
            </p>
            <div
              class="text-[11px] text-gray-400 font-bold uppercase tracking-widest mb-1"
            >
              Penerima
            </div>
            <div
