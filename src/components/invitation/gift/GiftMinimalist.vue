<template>
  <section v-if="bankList.length > 0" class="py-24 px-6 text-center bg-[var(--theme-surface)]">
    ...
    <div class="flex flex-col items-center gap-12 w-full">
      ...
      >
        <!-- Debit Card Concept (Minimalist) -->
        <div class="w-full aspect-[1.58] bg-gradient-to-br from-white/95 to-gray-50/90 p-6 rounded-2xl shadow-xl border border-white/60 relative overflow-hidden flex flex-col justify-between text-left backdrop-blur-md">
          
          <!-- Top Row: Chip and Bank Name -->
          <div class="flex justify-between items-start z-10">
            <!-- Chip -->
            <div class="w-11 h-8 bg-gradient-to-br from-[#e5c875] to-[#d4af37] rounded-md relative overflow-hidden shadow-inner border border-black/10">
            ...
            <div class="font-extrabold text-xl text-[var(--theme-primary)] tracking-wider uppercase">
              {{ bank.bank_name }}
            </div>
          </div>
          ...
          <div class="flex justify-between items-end z-10">
            <div class="flex flex-col">
              <span class="text-[10px] text-gray-500 font-semibold uppercase tracking-wider mb-1">A.N. Rekening</span>
              <span class="text-sm font-bold text-gray-700 uppercase tracking-wider leading-tight">
                {{ bank.bank_holder }}
              </span>
            </div>
            <!-- Debit Card Rings Mock -->
            <div class="flex opacity-70 items-center">
              <div class="w-8 h-8 rounded-full bg-red-500 mix-blend-multiply"></div>
              <div class="w-8 h-8 rounded-full bg-amber-500 mix-blend-multiply -ml-3.5"></div>
            </div>
          </div>
        </div>
