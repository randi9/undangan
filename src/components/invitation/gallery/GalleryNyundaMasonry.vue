<template>
  <section v-if="photos && photos.length > 0" style="padding: 60px 24px; text-align: center; background-color: #FFF8E7;">
    ...
    <div style="max-width: 500px; margin: 0 auto; display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
      <div
        v-for="(photo, index) in photos"
        :key="index"
        style="border-radius: 8px; overflow: hidden; cursor: pointer; background-color: #F5E6D0;"
        @click="$emit('openLightbox', index)"
      >
        <img :src="resolveAssetUrl(photo.url, apiBase)" :alt="photo.caption || 'Photo'" style="width: 100%; aspect-ratio: 1; object-fit: cover;" />
      </div>
    </div>
  </section>
</template>
