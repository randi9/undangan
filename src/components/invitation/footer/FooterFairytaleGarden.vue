<template>
  <footer ref="footerSection" class="py-24 px-6 text-center text-white relative overflow-hidden flex flex-col items-center justify-center min-h-[50vh]">
    ...
    <!-- Seamless Pattern Overlay (Botanical leaf path grid) -->
    <svg class="absolute inset-0 w-full h-full z-[2] opacity-10 pointer-events-none">
      <defs>
        <pattern id="fairytale-footer-pattern" x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
          <path d="M50 10 C54 20, 65 25, 65 40 C65 55, 54 60, 50 70 C46 60, 35 55, 35 40 C35 25, 46 20, 50 10Z" fill="#ffffff"/>
        </pattern>
      </defs>
      <rect width="100%" height="100%" fill="url(#fairytale-footer-pattern)" />
    </svg>
    ...
    <div ref="contentRef" class="relative z-10 opacity-0 translate-y-8 flex flex-col items-center">
      ...
      <p class="text-4xl md:text-6xl lg:text-7xl mb-12 drop-shadow-lg text-[#EBCFD1]" :style="{ fontFamily: themeConfig.fontHeading }">
...
function spawnSparkle() {
  ...
  // Fairytale colors: Pink and Gold-ish Cream
  const colors = ['rgba(235,207,209,', 'rgba(217,169,175,', 'rgba(248,243,238,'];
  const baseColor = colors[Math.floor(Math.random() * colors.length)] ?? 'rgba(235,207,209,';
  el.style.background = `${baseColor}${randomRange(0.4, 0.8)})`;
  el.style.boxShadow = `0 0 ${size * 2}px ${baseColor.replace(',', ' ')})`;
