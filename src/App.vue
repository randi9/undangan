...
<template>
  <header v-if="!$route.meta.requiresAuth && $route.name !== 'login'" class="p-4 sm:p-6 flex justify-end items-center fixed top-0 w-full z-50 pointer-events-none">
    <div class="pointer-events-auto bg-white/90 backdrop-blur-md rounded-full shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-gray-100 p-1.5 flex items-center gap-1 transition-all">
      <Show when="signed-out">
        <SignInButton mode="modal">
          <button class="text-sm font-semibold text-gray-600 hover:text-gray-900 px-4 py-2 rounded-full transition-colors">Sign In</button>
        </SignInButton>
        <SignUpButton mode="modal">
          <button class="bg-gray-900 text-white px-5 py-2 rounded-full text-sm font-semibold shadow-sm hover:bg-black transition-all hover:scale-105 active:scale-95">Sign Up</button>
        </SignUpButton>
      </Show>
      <Show when="signed-in">
        <div class="px-2 py-1">
